...
public class MessageView extends VerticalLayout {
    ...
    private Component getMessageBuilder() {
        List<WebPushSubscriptionEntity> allSubscriptions = service.getAllSubscriptions();
        ...
        MultiSelectComboBox<WebPushSubscriptionEntity> registeredUsers = new MultiSelectComboBox<>("Send to");
        registeredUsers.setItems(allSubscriptions);
        registeredUsers.setItemLabelGenerator(WebPushSubscriptionEntity::getUserName);
        ...
        Button send = new Button("Send", event -> new Thread(() -> {
            for (WebPushSubscriptionEntity subscription : registeredUsers.getSelectedItems()) {
